...
**Status atual (14/07/2026): PROJETO 1 ENCERRADO.** Fases 0 a 5 concluídas e documentadas. Fase 6 (empacotamento) com os 3 arquivos de `evaluation/` prontos para upload final no GitHub. Fase 7 (desligamento) concluída — infraestrutura AWS zerada. Próximo: Projeto 2 (agentes, MCP, guardrails).
...
## Fase 5 — Avaliação de qualidade (2 h, diferencial forte) ✅ CONCLUÍDA

**Decisão de metodologia:** RAGAS clássico não foi usado. Na tentativa de instalação, `langchain-community` (dependência do RAGAS) apresentou conflito real de versão com o restante do ambiente Python (o import `langchain_community.chat_models.vertexai` foi removido em versões recentes, e forçar uma versão antiga quebrava outras dependências). Em vez de uma gambiarra frágil, foi construída uma avaliação determinística própria (`evaluation/avaliar.py`): sem LLM-juiz, medindo **correção** (a resposta contém o fato esperado?) e **recuperação** (o documento-fonte esperado foi citado?) para 12 perguntas com gabarito conhecido, cobrindo os 6 documentos do corpus, incluindo um teste adversarial (produto inexistente) e um teste de precisão regulatória (data de contagem de um prazo retificado).

### Execução e achados reais (não hipotéticos)

A avaliação foi rodada três vezes, e cada rodada revelou um achado genuíno, não um erro de execução:

**Rodada 1 — 11/12 correção, 12/12 recuperação (92% / 100%).** A única falha (Q01) foi investigada: a resposta estava correta ("18 months"), mas em **inglês** — o gabarito procurava a string "18 meses" em português. Achado: o modelo alterna idioma de resposta sem instrução explícita de idioma-alvo, mesmo com pergunta em português.

**Rodada 2 (após ajuste pontual) — 9/12 correção, 12/12 recuperação (75% / 100%).** Duas novas perguntas "falharam" (Q06, Q08), pelo mesmo motivo: respostas corretas, de novo em inglês. Isso elevou o achado de "caso isolado" para **padrão real e não determinístico**: a mesma pergunta pode ser respondida em português numa execução e em inglês noutra, sem qualquer alteração na configuração do sistema entre as chamadas.

**Correção de metodologia:** o script foi generalizado para aceitar grupos de palavras-chave alternativos por idioma (`expected_keyword_groups`), validado contra as respostas reais capturadas nas rodadas anteriores antes de rodar de novo.

**Rodada 3 — 12/12 correção, 11/12 recuperação (100% / 92%).** A única divergência (Q05) não foi erro do sistema: a resposta citou o parecer original do DX-7 em vez do Manual de Procedimentos — mas esse parecer **também contém, literalmente, a regra de Classe II** ("com base... nos critérios da Seção 2 do ANVF-MP-001... 24 meses"), citada como justificativa própria. Achado: o corpus tem redundância legítima (a mesma regra aparece no manual e em cada parecer que a aplica), e um gabarito de fonte única por pergunta não captura isso. Corrigido com `expected_source_contains_any` (lista de fontes aceitas), validado offline contra a resposta real capturada.

### Resultado final

Com a metodologia corrigida (idioma bilíngue + fontes alternativas legítimas), as 12 perguntas passam integralmente, incluindo os casos que motivaram a correção. Latência média observada: ~1,6 a 1,8 segundos por consulta.

### Por que isso é mais forte do que "deu 100% de primeira"

Três achados reais e documentados, cada um com causa raiz distinta:
1. **Idioma de resposta não determinístico** — limitação real do sistema, não do avaliador. Registrada como evolução mapeada (padronizar idioma de saída).
2. **Redundância legítima de fontes no corpus** — não é limitação, é característica esperada de documentos regulatórios que se referenciam.
3. **RAGAS tem dependências frágeis no ecossistema LangChain atual** — decisão de engenharia documentada (avaliação determinística em vez de forçar uma biblioteca instável).

Arquivos: [`evaluation/dataset.json`](evaluation/dataset.json), [`evaluation/avaliar.py`](evaluation/avaliar.py), [`evaluation/resultado_avaliacao.json`](evaluation/resultado_avaliacao.json).
...
## Fase 7 — Desligar tudo (30 min, crítica) ✅ CONCLUÍDA

Ordem executada: Knowledge Base deletada → Lambda deletada → bucket S3 esvaziado e deletado → roles IAM do case deletadas (`nil-case-genai-lambda-role` e `AmazonBedrockExecutionRoleForKnowledgeBase_...`). Infraestrutura zerada em 14/07/2026. Custo final a confirmar no Cost Explorer após 24h, mas esperado próximo de zero, dado o uso pontual de recursos serverless/on-demand ao longo do projeto.

**Projeto 1 encerrado.** Todas as evidências (prints, código, resultados de avaliação) já estão documentadas no repositório; a infraestrutura não precisa mais existir para o case ser verificável.
