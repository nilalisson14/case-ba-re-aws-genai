...
**Status do projeto:** encerrado. Fases 0 a 7 concluídas (infraestrutura, dados, RAG funcional, API em produção, trilha de auditoria, avaliação de qualidade, empacotamento e desligamento). Infraestrutura AWS desprovisionada em 14/07/2026; todas as evidências permanecem documentadas neste repositório.
...
**Nota de metodologia:** RAGAS (biblioteca padrão de mercado) foi avaliado e descartado — sua dependência `langchain-community` tem um conflito real de versão no ecossistema LangChain atual (import removido em versões recentes). Em vez de forçar uma correção frágil, foi construída uma avaliação determinística própria: sem LLM-juiz, medindo correção factual e precisão de recuperação de fonte, para 12 perguntas com gabarito conhecido (baseado no próprio corpus sintético), incluindo um teste adversarial (produto inexistente, testando resistência a alucinação) e um teste de precisão regulatória (contagem de prazo retificado).

**Resultado final:** 12/12 corretas, 11/12 com recuperação de fonte esperada (a única divergência foi uma citação alternativa igualmente válida, ver achados abaixo). Latência média: ~1,6-1,8s por consulta.

**Três achados reais, cada um com causa raiz distinta:**

| Achado | Causa raiz | Classificação |
|---|---|---|
| Idioma de resposta inconsistente (PT/EN alternando sem instrução) | Modelo não recebeu instrução de idioma-alvo na configuração | Limitação real do sistema — evolução mapeada |
| Uma pergunta "falhou" recuperação, mas citou fonte alternativa válida | Corpus tem redundância legítima (regra do manual repetida nos pareceres que a aplicam) | Não é falha — ajuste de metodologia de avaliação |
| RAGAS não instalou de forma estável | Conflito de dependências no ecossistema LangChain atual | Decisão de engenharia documentada |
...
O passo a passo completo de execução, incluindo decisões técnicas e problemas encontrados durante a implementação, está documentado em [`roadmap/passo_a_passo.md`](roadmap/passo_a_passo.md). A infraestrutura foi desprovisionada em 14/07/2026 (Knowledge Base, Lambda, bucket S3 e roles IAM do case), após a coleta de todas as evidências deste README. O ambiente é 100% reproduzível seguindo o roadmap, do zero, em poucas horas.
